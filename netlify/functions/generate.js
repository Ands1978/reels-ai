...
function localScripts(idea, count) {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return { titulo: `${a[0]}: ${topic}`, hook: a[1], body: a[2], example: a[3], action: a[4], close: a[5] };
  });
}
...
async function callClaude({ idea, duration, style, count }) {
  ...
  const response = await fetch(`${baseURL.replace(/\/$/, "")}/v1/messages`, {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: `Tema do Reel: ${idea}` }]
    })
  });
...
async function callPerplexity({ idea, duration, style, count }) {
  ...
  const response = await fetch("https://api.perplexity.ai/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: "system", content: generationInstructions({ idea, duration, style, count }) + " Pesquise apenas quando fatos atuais forem necessários." },
        { role: "user", content: `Tema do Reel: ${idea}` }
      ],
      temperature: 0.9
    })
  });
...
async function callOpenAI({ idea, duration, style, count }) {
  ...
  const response = await client.responses.create({
    ...
    temperature: 0.9,
    text: { format: { type: "json_schema", name: "reels_generation", strict: true, schema: { type: "object", additionalProperties: false, properties: { reels: { type: "array", minItems: 1, maxItems: 10, items: { type: "object", additionalProperties: false, properties: { titulo: { type: "string" }, hook: { type: "string" }, body: { type: "string" }, example: { type: "string" }, action: { type: "string" }, close: { type: "string" } }, required: ["titulo", "hook", "body", "example", "action", "close"] } } }, required: ["reels"] } } }
  });
...
async function generateWithAI({ idea, duration, style, count }) {
  ...
  if (process.env.ALLOW_LOCAL_FALLBACK !== "false") {
    return { items: localScripts(idea, count), mode: "local-fallback", warning: errors.join(" | ") || "Nenhum provedor de IA disponível." };
  }
...
exports.handler = async function(event) {
  try {
    ...
    let data;
    try { data = JSON.parse(event.body || "{}"); } catch (_) { return { statusCode: 400, headers: { "content-type": "application/json" }, body: JSON.stringify({ error: "JSON inválido." }) }; }
    ...
    const items = result.items.map((script, index) => ({
    ...
    return {
      ...
      body: JSON.stringify({ id: require("crypto").randomUUID(), idea, style, duration, count: items.length, mode: result.mode, warning: result.warning || null, items })
    };
  } catch (error) {
    console.error(error);
