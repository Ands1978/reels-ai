...
const STYLE_TEMPLATES = {
  viral: {
    ...
    titles: ["O ponto que chama atenção em {tema}", "O erro mais comum em {tema}", "O detalhe de {tema} que você precisa ver", "Como simplificar {tema}"],
    bodies: ["A ideia principal é simples: {tema} fica mais fácil quando você começa pelo que realmente importa.", "Em vez de tentar fazer tudo ao mesmo tempo, escolha um ponto, teste e observe o resultado.", "Um exemplo prático é transformar essa ideia em uma pequena ação que você consegue testar hoje.", "O próximo passo é colocar isso em prática e ajustar com base no que funcionar melhor."],
  ...
  educativo: {
    ...
    titles: ["O básico que você precisa saber sobre {tema}", "Como entender {tema} passo a passo", "3 pontos importantes sobre {tema}", "Como aplicar {tema} na prática"],
    bodies: ["Primeiro, entenda o conceito central. Depois, divida o assunto em partes menores para facilitar a aplicação.", "Um bom exemplo é começar com uma situação real e observar como {tema} funciona nesse contexto.", "Depois de entender a teoria, escolha uma pequena tarefa e aplique o conceito nela.", "Assim você transforma conhecimento em prática e consegue identificar o que ainda precisa melhorar."],
  ...
  motivacional: {
    ...
    titles: ["Comece hoje com {tema}", "O primeiro passo em {tema}", "Como sair da teoria em {tema}", "Uma pequena ação sobre {tema}"],
    bodies: ["Você não precisa resolver tudo de uma vez. Escolha uma única ação que aproxime você do resultado que procura.", "Se algo der errado, use o resultado como informação para ajustar o próximo passo.", "Por exemplo, reserve alguns minutos hoje para testar uma parte de {tema} em uma situação real.", "Consistência vale mais do que esperar pelo momento perfeito."],
  ...
  storytelling: {
    ...
    titles: ["Uma história sobre {tema}", "O problema que explica {tema}", "Como uma situação comum revela {tema}", "A lição por trás de {tema}"],
    bodies: ["Imagine uma pessoa diante de um problema real. Em vez de procurar uma solução perfeita, ela começa pelo passo mais simples.", "Depois do primeiro teste, ela percebe o que funcionou e muda a estratégia onde precisava.", "É aí que {tema} deixa de ser apenas uma ideia e passa a fazer sentido na prática.", "A lição é simples: começar, observar e ajustar costuma ser mais útil do que ficar apenas planejando."],
...
function createScript(idea, duration, variation = 0, style = "viral") {
  ...
  return {
    ...
    tema,
    style: mode,
    scenes: scenes.map((text, index) => ({ start: Number((index * step).toFixed(1)), end: Number(((index + 1) * step).toFixed(1)), text }))
  };
}
